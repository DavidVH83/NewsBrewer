...
Usage::
    ...
    python setup.py --help     # Show usage information
    python setup.py --check    # Validate existing config.yml and .env
"""
...
def write_config_yml(config_data: dict[str, Any], output_path: str) -> None:
    ...
    with open(output_path, "w", encoding="utf-8") as fh:
        fh.write("# ============================================================\n")
        fh.write("# NEWSBREWER CONFIGURATION — generated by setup.py\n")
...
def check_existing_config(project_root: str) -> None:
    ...
    if errors:
        ...
        for e in errors:
            print(f"  x {e}")
        print("\nStatus: CONFIGURATION HAS ERRORS — run 'python setup.py' to fix.")
        sys.exit(1)
    else:
        if warnings:
...
def show_help() -> None:
    ...
    print(__doc__)
    print("Commands:")
    print("  python setup.py          Run the interactive setup wizard")
    print("  python setup.py --help   Show this help message")
    print("  python setup.py --check  Validate existing config.yml and .env")
    print()
